...
const SectionHeader=({icon,title,desc})=>(
  <div className="flex items-center gap-2.5 mb-3">
    ...
    <div>
      <h4 className="text-[12px] font-semibold text-[var(--sb-text-strong)] leading-tight">{title}</h4>
...
const lc="flex items-center gap-1.5 text-[12px] font-semibold text-[var(--sb-text)] mb-1.5";
const hint="text-[11px] text-[var(--sb-muted-soft)] mt-1 leading-relaxed";
...
const Field=({label,required,hintText,tooltip,error,errorMsg,children,className=''})=>(
  <div className={className}>
    <label className={lc}>
      ...
      {tooltip&&<Tip text={tooltip}><span className="text-[var(--sb-muted-soft)] hover:text-[var(--sb-muted)] cursor-help transition-colors"><I n="info" s={12}/></span></Tip>}
    </label>
    {children}
...
const UpZ=({value,onUpload,onRemove,label,accept,hintText})=>{
  ...
  if(up){
    ...
    return <div className="rounded-md p-3 flex items-center gap-3 group" style={{background:'rgba(var(--sb-accent-rgb),0.08)',border:'1px solid rgba(var(--sb-accent-rgb),0.25)'}}>
      <div className="w-8 h-8 rounded-md flex items-center justify-center flex-shrink-0" style={{background:'rgba(var(--sb-accent-rgb),0.15)'}}><I n="file" s={14} c="text-[var(--sb-accent)]"/></div>
      <div className="flex-1 min-w-0"><p className="text-[12px] font-medium text-[var(--sb-accent)] truncate">{fl[0]}</p><p className="text-[11px]" style={{color:'rgba(var(--sb-accent-rgb),0.6)'}}>Uploaded successfully</p></div>
      <button onClick={e=>{e.stopPropagation();onRemove()}} className="p-1 rounded-md text-[var(--sb-muted-soft)] transition-all opacity-0 group-hover:opacity-100" style={{}} onMouseEnter={e=>{e.currentTarget.style.background='rgba(239,68,68,0.1)';e.currentTarget.style.color='#f87171'}} onMouseLeave={e=>{e.currentTarget.style.background='';e.currentTarget.style.color='var(--sb-muted-soft)'}}><I n="trash" s={13}/></button>
    </div>;
  }
  return <div onClick={onUpload} className="intake-upload-zone rounded-md p-5 text-center cursor-pointer group" style={{borderWidth:2}}>
    <div className="w-9 h-9 rounded-md bg-[var(--sb-panel-2)] group-hover:bg-[rgba(var(--sb-accent-rgb),0.1)] flex items-center justify-center mx-auto mb-2 transition-colors"><I n="upload" s={16} c="text-[var(--sb-muted-soft)] group-hover:text-[var(--sb-accent)] transition-colors"/></div>
    <p className="text-[12px] font-medium text-[var(--sb-muted)] group-hover:text-[var(--sb-accent)] transition-colors">Click to upload {label.toLowerCase()}</p>
    <p className="text-[11px] text-[var(--sb-muted-soft)] mt-1">{accept||'Any file type'}</p>
    {hintText&&<p className="text-[11px] text-[var(--sb-muted-soft)] mt-0.5">{hintText}</p>}
  </div>;
};
  ...
  <div className={`${columns?`grid ${gridColsClass}`:'flex flex-wrap'} gap-1.5 ${errState?'ring-1 ring-rose-500/20 rounded-md p-1':''}`}>
    {options.map(o=>{
      ...
      return <button key={o} type="button" onClick={()=>onToggle(o)}
        ...
        <div className={`w-3.5 h-3.5 rounded flex items-center justify-center transition-all`} style={{borderWidth:'1.5px',borderStyle:'solid',...(active?{background:'var(--sb-accent)',borderColor:'var(--sb-accent)'}:{borderColor:'var(--sb-muted-soft)'})}}>
...
const IntakeForm=({onSubmit,onViewCampaign})=>{
  ...
  const priorityOpts=[
    {v:'High',desc:'Launch-critical',bg:'rgba(239,68,68,0.08)',bc:'rgba(239,68,68,0.35)',ring:'rgba(239,68,68,0.15)',dot:'#ef4444',label:'var(--sb-pri-high)',hint:'var(--sb-pri-high-soft)'},
    {v:'Medium',desc:'Standard timeline',bg:'rgba(245,158,11,0.08)',bc:'rgba(245,158,11,0.35)',ring:'rgba(245,158,11,0.15)',dot:'#f59e0b',label:'var(--sb-pri-med)',hint:'var(--sb-pri-med-soft)'},
    {v:'Low',desc:'Flexible timing',bg:'rgba(56,189,248,0.08)',bc:'rgba(56,189,248,0.35)',ring:'rgba(56,189,248,0.15)',dot:'#38bdf8',label:'var(--sb-pri-low)',hint:'var(--sb-pri-low-soft)'},
  ];

  if(submitted)return <div className="flex items-center justify-center py-20 anim-scale"><div className="text-center max-w-md">
    <div className="w-14 h-14 rounded-xl flex items-center justify-center mx-auto mb-4" style={{background:'rgba(var(--sb-accent-rgb),0.12)'}}><I n="check" s={24} c="text-[var(--sb-accent)]"/></div>
    <h2 className="text-lg font-semibold text-[var(--sb-text-strong)]">Campaign Submitted</h2>
    <p className="text-[13px] text-[var(--sb-muted)] mt-2 leading-relaxed"><strong className="text-[var(--sb-text-strong)]">"{fd.projectName}"</strong> has been submitted successfully. A confirmation email will be sent to all stakeholders.</p>
    <p className="text-[11px] text-[var(--sb-muted-soft)] mt-3 font-mono">Reference: IC-2026-{String(CAMPS.length+1).padStart(4,'0')}</p>
    <div className="flex items-center justify-center gap-3 mt-5">
      {newCampaignId&&<button onClick={()=>onViewCampaign?.(newCampaignId)} className="px-5 py-2 rounded-md text-[13px] font-semibold transition-colors" style={{background:'var(--sb-accent)',color:'var(--sb-accent-contrast)'}} onMouseEnter={e=>e.currentTarget.style.background='var(--sb-accent-strong)'} onMouseLeave={e=>e.currentTarget.style.background='var(--sb-accent)'}>View Campaign →</button>}
      <button onClick={()=>{setSubmitted(false);setIsSubmitting(false);setNewCampaignId(null);setStep(0);setDir(1);setFd({...INIT_FD});setAttempted(new Set());setTouched({})}} className="px-5 py-2 rounded-md text-[13px] font-medium transition-colors" style={{border:'1px solid var(--sb-border)',color:'var(--sb-text)'}} onMouseEnter={e=>e.currentTarget.style.background='var(--sb-panel-2)'} onMouseLeave={e=>e.currentTarget.style.background=''}>Submit Another</button>
    </div>
  </div></div>;
  ...
  const renderStep=()=>{
    switch(step){
      case 0:return <>
        <p className="text-[11px] text-[var(--sb-muted-soft)] mb-2">Intake Date: {intakeDate}</p>
      ...
      case 1:return <>
            ...
            <div className={errA(fd.banners)?'ring-1 ring-rose-500/20 rounded-md p-1':''}>
              <div className="flex items-center justify-between mb-2">
                <button type="button" onClick={()=>togBanner('All')}
                  ...
                  style={{color:fd.banners.length===BANNERS.length?'var(--sb-accent)':'var(--sb-muted)'}}
                  onMouseEnter={e=>{if(fd.banners.length!==BANNERS.length)e.currentTarget.style.color='var(--sb-text)'}}
                  onMouseLeave={e=>{if(fd.banners.length!==BANNERS.length)e.currentTarget.style.color='var(--sb-muted)'}}>
                  <div className="w-3.5 h-3.5 rounded flex items-center justify-center transition-all" style={{borderWidth:'1.5px',borderStyle:'solid',...(fd.banners.length===BANNERS.length?{background:'var(--sb-accent)',borderColor:'var(--sb-accent)'}:{borderColor:'var(--sb-muted-soft)'})}}>
                ...
                {fd.banners.length>0&&<span className="text-[11px] text-[var(--sb-muted-soft)]">{fd.banners.length} of {BANNERS.length} selected</span>}
              </div>
              <div className="grid grid-cols-4 gap-1.5">
                {BANNERS.map(b=>{
                  ...
                  return <button key={b} type="button" onClick={()=>togBanner(b)}
                    ...
                    <div className="w-3.5 h-3.5 rounded flex items-center justify-center transition-all flex-shrink-0" style={{borderWidth:'1.5px',borderStyle:'solid',...(active?{background:'var(--sb-accent)',borderColor:'var(--sb-accent)'}:{borderColor:'var(--sb-muted-soft)'})}}>
                      {active&&<svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3" strokeLinecap="round"><polyline points="20 6 9 17 4 12"/></svg>}
                    </div>
        ...
        <div className="border-t border-[var(--sb-border)] mt-5 pt-5">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <button type="button" onClick={()=>{setRepMode('auto');upd('rep',MKT_RES[fd.mr]||'')}}
                  ...
                  style={{borderWidth:2,borderStyle:'solid',...(repMode==='auto'?{background:'rgba(var(--sb-accent-rgb),0.08)',borderColor:'rgba(var(--sb-accent-rgb),0.35)'}:{borderColor:'var(--sb-border)'})}}
                  onMouseEnter={e=>{if(repMode!=='auto'){e.currentTarget.style.borderColor='var(--sb-border)';e.currentTarget.style.background='var(--sb-panel-2)'}}}
                  onMouseLeave={e=>{if(repMode!=='auto'){e.currentTarget.style.borderColor='var(--sb-border)';e.currentTarget.style.background=''}}}>
                  <p className="text-[11px] font-semibold mb-0.5" style={{color:repMode==='auto'?'var(--sb-accent)':'var(--sb-text)'}}>Auto-assign</p>
                  <p className="text-[11px]" style={{color:repMode==='auto'?'rgba(var(--sb-accent-rgb),0.6)':'var(--sb-muted-soft)'}}>Default rep for this team</p>
                </button>
                <button type="button" onClick={()=>{setRepMode('custom');upd('rep','')}}
                  className="px-3 py-2.5 rounded-lg text-left transition-all"
                  style={{borderWidth:2,borderStyle:'solid',...(repMode==='custom'?{background:'rgba(var(--sb-accent-rgb),0.08)',borderColor:'rgba(var(--sb-accent-rgb),0.35)'}:{borderColor:'var(--sb-border)'})}}
                  onMouseEnter={e=>{if(repMode!=='custom'){e.currentTarget.style.borderColor='var(--sb-border)';e.currentTarget.style.background='var(--sb-panel-2)'}}}
                  onMouseLeave={e=>{if(repMode!=='custom'){e.currentTarget.style.borderColor='var(--sb-border)';e.currentTarget.style.background=''}}}>
                  <p className="text-[11px] font-semibold mb-0.5" style={{color:repMode==='custom'?'var(--sb-accent)':'var(--sb-text)'}}>Custom</p>
                  <p className="text-[11px]" style={{color:repMode==='custom'?'rgba(var(--sb-accent-rgb),0.6)':'var(--sb-muted-soft)'}}>Assign to someone else</p>
                </button>
              </div>
            </div>
            {repMode==='auto'&&fd.rep&&<div className="flex items-center gap-3 rounded-lg px-4 py-3 anim-scale" style={{background:'rgba(var(--sb-accent-rgb),0.08)',border:'1px solid rgba(var(--sb-accent-rgb),0.25)'}}>
              <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0" style={{background:'rgba(var(--sb-accent-rgb),0.15)'}}><span className="text-[11px] font-bold text-[var(--sb-accent)]">{fd.rep.split(' ').map(x=>x[0]).join('')}</span></div>
              <div className="flex-1"><p className="text-[13px] font-semibold text-[var(--sb-text-strong)]">{fd.rep}</p></div>
              <div className="w-5 h-5 rounded-full flex items-center justify-center" style={{background:'rgba(var(--sb-accent-rgb),0.15)'}}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--sb-accent)" strokeWidth="3" strokeLinecap="round"><polyline points="20 6 9 17 4 12"/></svg></div>
            </div>}
          ...
          <Field label="Priority Level" required error={err(fd.pri)} errorMsg="Select a priority level">
            <div className={`grid grid-cols-3 gap-2 ${err(fd.pri)?'ring-1 ring-rose-500/20 rounded-md p-1':''}`}>
              {priorityOpts.map(p=><button key={p.v} type="button" onClick={()=>upd('pri',p.v)}
                ...
                onMouseEnter={e=>{if(fd.pri!==p.v){e.currentTarget.style.borderColor='var(--sb-muted-soft)';e.currentTarget.style.background='var(--sb-panel-2)'}}}
                onMouseLeave={e=>{if(fd.pri!==p.v){e.currentTarget.style.borderColor='var(--sb-border)';e.currentTarget.style.background=''}}}>
                <div className="flex items-center gap-2 mb-0.5">
                  <div className="w-2 h-2 rounded-full" style={{background:fd.pri===p.v?p.dot:'var(--sb-muted-soft)'}}/>
                  <span className="text-[13px] font-semibold" style={{color:fd.pri===p.v?p.label:'var(--sb-text)'}}>{p.v}</span>
                </div>
                <p className="text-[11px] ml-4" style={{color:fd.pri===p.v?p.hint:'var(--sb-muted-soft)'}}>{p.desc}</p>
              </button>)}
            </div>
          </Field>
          <div className="grid grid-cols-2 gap-3">
      ...
      case 5:return <>
        ...
        <div className="space-y-4">
          ...
          {chLeads.length>0&&<div>
            <label className={lc}><I n="user" s={11} c="text-[var(--sb-muted-soft)]"/>Channel Leads</label>
            <p className="text-[11px] text-[var(--sb-muted-soft)] mb-3">Auto-assigned based on channel selection.</p>
            <div className="space-y-1.5">{chLeads.map(({ch,lead})=>{
              ...
              return <div key={ch} className="flex items-center gap-3 rounded-md px-3 py-2" style={{background:'var(--sb-bg-soft)',border:`1px solid ${isCustom?'rgba(var(--sb-accent-rgb),0.25)':'var(--sb-border)'}`}}>
                <span className="text-[11px] text-[var(--sb-muted)] w-[100px] flex-shrink-0 truncate">{ch}</span>
                <div className="flex-1 min-w-0">
                  {isEditing
                    ...
                    :<div className="flex items-center gap-2">
                      <span className="text-[12px] font-medium text-[var(--sb-text-strong)] truncate">{current}</span>
                      {isCustom&&<span className="text-[11px] px-1.5 py-0.5 rounded-full font-medium flex-shrink-0" style={{background:'rgba(var(--sb-accent-rgb),0.1)',color:'var(--sb-accent)'}}>Custom</span>}
                    </div>}
                </div>
                {isEditing
                  ?<div className="flex items-center gap-0.5 flex-shrink-0">
                    <button type="button" onClick={()=>setEditingLeads(p=>{const n=new Set(p);n.delete(ch);return n})}
                      className="p-1 rounded hover:bg-[rgba(var(--sb-accent-rgb),0.12)] text-[var(--sb-accent)] transition-colors" title="Save"><I n="check" s={11}/></button>
                    <button type="button" onClick={()=>{setFd(p=>{const o={...p.chLeadOverrides};delete o[ch];return{...p,chLeadOverrides:o}});setEditingLeads(p=>{const n=new Set(p);n.delete(ch);return n})}}
                      className="p-1 rounded hover:bg-[rgba(244,63,94,0.1)] text-[var(--sb-muted-soft)] hover:text-[#f87171] transition-colors" title="Reset to default"><I n="x" s={11}/></button>
                  </div>
                  :<button type="button" onClick={()=>setEditingLeads(p=>{const n=new Set(p);n.add(ch);return n})}
                    className="text-[11px] font-medium text-[var(--sb-muted-soft)] hover:text-[var(--sb-accent)] transition-colors flex-shrink-0">Edit</button>}
              </div>;
            })}</div>
          </div>}
        </div>
          ...
          <Field label="Campaign Type" required tooltip="New: first-time. Existing: returning. Carry Forward: extension. Evergreen: always-on." error={err(fd.campType)} errorMsg="Select a campaign type">
            <div className="grid grid-cols-4 gap-2">
              {[
                ...
                style={{borderWidth:2,borderStyle:'solid',...(fd.campType===t.v?{background:'rgba(var(--sb-accent-rgb),0.08)',borderColor:'rgba(var(--sb-accent-rgb),0.35)',color:'var(--sb-accent)'}:{borderColor:'var(--sb-border)',color:'var(--sb-muted)'})}}
                onMouseEnter={e=>{if(fd.campType!==t.v){e.currentTarget.style.borderColor='var(--sb-muted-soft)';e.currentTarget.style.background='var(--sb-panel-2)'}}}
                onMouseLeave={e=>{if(fd.campType!==t.v){e.currentTarget.style.borderColor='var(--sb-border)';e.currentTarget.style.background=''}}}>
                <I n={t.icon} s={15} c={`mx-auto mb-1 ${fd.campType===t.v?'text-[var(--sb-accent)]':'text-[var(--sb-muted-soft)]'}`}/>
                <p className="text-[12px] font-semibold">{t.v}</p>
                <p className="text-[11px] mt-0.5" style={{color:fd.campType===t.v?'rgba(var(--sb-accent-rgb),0.6)':'var(--sb-muted-soft)'}}>{t.desc}</p>
              </button>)}
            </div>
          </Field>
          <div style={{borderTop:'1px solid var(--sb-border)',paddingTop:16}}>
            <p className="text-[11px] font-semibold text-[var(--sb-muted)] uppercase tracking-wide mb-2.5">Audience Targeting</p>
      ...
      case 7:return <>
        ...
        <div className="space-y-4">
          ...
          {fd.merchFiles.length>0&&<div className="rounded-lg p-3.5 anim-scale" style={{background:'rgba(var(--sb-accent-rgb),0.08)',border:'1px solid rgba(var(--sb-accent-rgb),0.25)'}}>
            <p className="text-[11px] font-semibold text-[var(--sb-accent)] uppercase tracking-wide mb-2.5 flex items-center gap-1.5"><I n="zap" s={11} c="text-[var(--sb-accent)]"/>Auto-extracted Data</p>
            <div className="grid grid-cols-2 gap-2 text-[12px]">
              {[['Merch Status','Accepted','check'],['UPC List','3 items','hash'],['U Offer IDs','UOF-2026-0451, 0452','file'],['4x Offer IDs','4X-2026-0891','file'],['Stock Up Event','SU-2026-Q1-003','flag']].map(([k,v,icon])=>
                <div key={k} className="flex items-center gap-2 rounded-md px-2.5 py-1.5" style={{background:'var(--sb-panel)',border:'1px solid rgba(var(--sb-accent-rgb),0.2)'}}>
                  <I n={icon} s={12} c="text-[var(--sb-accent)]"/>
                  <div><p className="text-[11px] font-medium" style={{color:'rgba(var(--sb-accent-rgb),0.6)'}}>{k}</p><p className="text-[11px] font-semibold text-[var(--sb-text-strong)]">{v}</p></div>
                </div>
              )}
            </div>
          </div>}
      ...
      case 8:return <>
        {hasMedia?<>
          ...
          <div className="w-9 h-9 rounded-lg bg-[var(--sb-panel-2)] flex items-center justify-center mx-auto mb-2"><I n="layers" s={16} c="text-[var(--sb-muted-soft)]"/></div>
          <p className="text-[13px] font-medium text-[var(--sb-muted)]">Media Collective not required</p>
          <p className="text-[11px] text-[var(--sb-muted-soft)] mt-1">Enable Media Collective in Channel Support (Step 6) to upload vendor materials.</p>
        </div>}
        <div className="border-t border-[var(--sb-border)] mt-5 pt-5">
            ...
            <Field label="Creative Development Type" required tooltip="In-Store: physical displays. Digital: email, web, app." error={err(fd.creativeType)} errorMsg="Select a creative type">
              <div className="grid grid-cols-3 gap-2">
                {['In-Store','Digital','Both'].map(t=><button key={t} type="button" onClick={()=>upd('creativeType',t)}
                  ...
                  style={{borderWidth:2,borderStyle:'solid',...(fd.creativeType===t?{background:'rgba(var(--sb-accent-rgb),0.08)',borderColor:'rgba(var(--sb-accent-rgb),0.35)',color:'var(--sb-accent)',fontWeight:600}:{borderColor:'var(--sb-border)',color:'var(--sb-muted)',fontWeight:500})}}
                  onMouseEnter={e=>{if(fd.creativeType!==t){e.currentTarget.style.borderColor='var(--sb-muted-soft)';e.currentTarget.style.background='var(--sb-panel-2)'}}}
                  onMouseLeave={e=>{if(fd.creativeType!==t){e.currentTarget.style.borderColor='var(--sb-border)';e.currentTarget.style.background=''}}}>
                  {t}
                </button>)}
              </div>
            </Field>
      ...
      case 9:{
        ...
        return <>
          ...
          {secs.map(([si,t,icon,items])=>{
            ...
            return <div key={t} className="rounded-lg overflow-hidden" style={{border:'1px solid var(--sb-border)'}}>
              <div className="px-4 py-2.5 flex items-center justify-between" style={{borderBottom:'1px solid var(--sb-border)'}}>
                <div className="flex items-center gap-2">
                  <I n={icon} s={13} c="text-[var(--sb-muted-soft)]"/>
                  <span className="text-[12px] font-semibold text-[var(--sb-text-strong)]">{t}</span>
                </div>
                <button onClick={()=>goTo(si)} className="text-[11px] font-medium flex items-center gap-1 transition-colors" style={{color:'var(--sb-accent)'}} onMouseEnter={e=>{e.currentTarget.style.color='var(--sb-accent-strong)';e.currentTarget.style.textDecoration='underline'}} onMouseLeave={e=>{e.currentTarget.style.color='var(--sb-accent)';e.currentTarget.style.textDecoration='none'}}><I n="edit" s={11}/>Edit</button>
              </div>
              <div className="px-4 py-3">
                <dl className="grid grid-cols-2 gap-x-5 gap-y-2.5">
                  {filled.map(([k,v])=><div key={k}>
                    <dt className="text-[11px] font-medium text-[var(--sb-muted-soft)] uppercase tracking-wide">{k}</dt>
                    <dd className="text-[12px] text-[var(--sb-text-strong)] font-medium mt-0.5 leading-relaxed">{v}</dd>
                  </div>)}
                </dl>
              </div>
            </div>;
  ...
  return <div className="h-full min-h-0 flex flex-col">
    <div className="flex-1 min-h-0 grid grid-cols-1 xl:grid-cols-[260px_minmax(0,1fr)] gap-3">
      {/* ═══ SIDEBAR ═══ */}
      <aside className="min-w-0 rounded-lg p-3 overflow-y-auto scrollbar-thin" style={{background:'var(--sb-bg-soft)',border:'1px solid var(--sb-border)'}}>
        <div className="mb-4">
          <div className="flex items-center justify-between mb-1.5">
            <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-[var(--sb-muted-soft)]">Progress</p>
            <span className="text-[11px] font-semibold text-[var(--sb-accent)]">{progress}%</span>
          </div>
          <div className="h-[3px] rounded-full" style={{background:'var(--sb-border)'}}>
            <div className="h-full rounded-full progress-bar" style={{width:`${progress}%`,background:'var(--sb-accent)'}}/>
          </div>
        </div>
        {STEP_GROUPS.map(g=>(
          <div key={g.label} className="mb-3">
            <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-[var(--sb-muted-soft)] mb-1.5 px-2">{g.label}</p>
            {g.steps.map(i=>{
              ...
              return <button key={i} type="button" onClick={()=>{if(isClickable)goTo(i)}}
                ...
                style={{borderLeft:`2px solid ${isCurrent?'var(--sb-accent)':isDone?'rgba(var(--sb-accent-rgb),0.4)':'transparent'}`}}
                onMouseEnter={e=>{if(isClickable&&!isCurrent)e.currentTarget.style.background='var(--sb-panel-2)'}}
                onMouseLeave={e=>{e.currentTarget.style.background=''}}>
                <span className="w-5 h-5 rounded-full flex items-center justify-center text-[11px] font-semibold flex-shrink-0"
                  style={{...(isDone?{background:'rgba(var(--sb-accent-rgb),0.12)',color:'var(--sb-accent)'}:isCurrent?{background:'var(--sb-accent)',color:'var(--sb-accent-contrast)'}:{background:'var(--sb-panel-2)',border:'1px solid var(--sb-border)',color:'var(--sb-muted-soft)'})}}>
                  {isDone?<I n="check" s={10}/>:i+1}
                </span>
                <span className={`text-[12px] ${isCurrent?'text-[var(--sb-text-strong)] font-semibold':isDone?'text-[var(--sb-muted)] font-medium':'text-[var(--sb-muted)] font-medium'}`}>
                  {STEPS[i].name}
                </span>
              </button>;
            })}
      ...
      {/* ═══ STEP CONTENT ═══ */}
      <section className="min-w-0 rounded-lg flex flex-col overflow-hidden" style={{background:'var(--sb-panel)',border:'1px solid var(--sb-border)'}}>
        <div className="px-5 py-3" style={{borderBottom:'1px solid var(--sb-border)',background:'var(--sb-panel-2)'}}>
          <div className="flex items-center gap-2.5">
            <span className="w-7 h-7 rounded-md flex items-center justify-center text-[var(--sb-accent)] flex-shrink-0" style={{background:'rgba(var(--sb-accent-rgb),0.1)',border:'1px solid rgba(var(--sb-accent-rgb),0.25)'}}>
              <I n={STEPS[step].icon} s={14}/>
            </span>
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <h3 className="text-[15px] font-semibold text-[var(--sb-text-strong)] leading-tight">{STEPS[step].name}</h3>
                <span className="text-[11px] text-[var(--sb-muted-soft)]">Step {step+1}/{STEPS.length}</span>
              </div>
              <p className="text-[11px] text-[var(--sb-muted)] mt-0.5 truncate">{STEPS[step].desc}</p>
            </div>
          </div>
        </div>
        <div className="h-[2px]" style={{background:'var(--sb-border)'}}>
          <div className="h-full progress-bar" style={{width:`${progress}%`,background:'var(--sb-accent)'}}/>
        </div>
    ...
    <div className="intake-shell-footer sticky bottom-0 mt-2 rounded-md px-3 py-2 flex items-center" style={{border:'1px solid var(--sb-border)'}}>
      ...
      {(isCurrentStepValid||!attempted.has(step)||currentReqs.length===0)&&<span className="mr-auto text-[11px] text-[var(--sb-muted-soft)] hidden sm:inline">Press <kbd className="kbd">Enter</kbd> to continue</span>}
      <div className="flex items-center gap-2">
        <button type="button" onClick={prev} disabled={step===0}
          className="px-3 py-1.5 rounded-md text-[12px] font-medium transition-colors"
          style={{border:'1px solid',...(step>0?{borderColor:'var(--sb-border)',color:'var(--sb-text)'}:{borderColor:'var(--sb-border)',background:'var(--sb-panel-3)',color:'var(--sb-muted-soft)',cursor:'not-allowed'})}}
          onMouseEnter={e=>{if(step>0)e.currentTarget.style.background='var(--sb-panel-2)'}}
        ...
        {step<9&&<button type="button" data-next-btn onClick={next} disabled={!isCurrentStepValid}
          ...
          style={isCurrentStepValid?{background:'var(--sb-accent)',color:'var(--sb-accent-contrast)'}:{background:'rgba(var(--sb-accent-rgb),0.2)',color:'rgba(var(--sb-accent-rgb),0.4)',cursor:'not-allowed'}}
          onMouseEnter={e=>{if(isCurrentStepValid)e.currentTarget.style.background='var(--sb-accent-strong)'}}
          onMouseLeave={e=>{if(isCurrentStepValid)e.currentTarget.style.background='var(--sb-accent)'}}>Next</button>}
        {step===9&&<button type="button" onClick={submit} disabled={isSubmitting}
          className="px-3.5 py-1.5 rounded-md text-[12px] font-semibold transition-colors"
          style={isSubmitting?{background:'rgba(var(--sb-accent-rgb),0.2)',color:'rgba(var(--sb-accent-rgb),0.4)',cursor:'wait'}:{background:'var(--sb-accent)',color:'var(--sb-accent-contrast)'}}
          onMouseEnter={e=>{if(!isSubmitting)e.currentTarget.style.background='var(--sb-accent-strong)'}}
          onMouseLeave={e=>{if(!isSubmitting)e.currentTarget.style.background='var(--sb-accent)'}}>{isSubmitting?'Creating...':'Create Campaign'}</button>}
      </div>
    </div>
  </div>;
};
...
export default function IntakePage({ onSubmit, onInput, onViewCampaign }) {
  return <div className="anim-fade h-[calc(100vh-148px)] flex flex-col min-h-0" onInput={onInput}>
    <div className="mb-4 shrink-0">
      <h1 className="text-[22px] font-semibold tracking-tight text-[var(--sb-text-strong)]">New Campaign Request</h1>
      <p className="text-[13px] text-[var(--sb-muted-soft)]">Complete the intake form to submit a new marketing campaign for review.</p>
    </div>
